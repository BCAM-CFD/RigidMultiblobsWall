...
from plot import plot_velocity_field_on_sphere as pvf
...
class QuaternionIntegrator(object):
  ...
  def __init__(self, bodies, Nblobs, scheme, tolerance = None, domain = 'single_wall'): 
    ...
    self.order_multipoles = 7
    self.U_bacteria = np.zeros((2, 3))
    self.U_bacteria_averaged = np.zeros((12))     
    self.X_bacteria_averaged = np.zeros((((np.power(3, self.order_multipoles+1))-1)//2+2,3)) 
  ...
  def articulated_deterministic_forward_euler(self, dt, *args, **kwargs):
    ...
    while True:      
      ...
      lambda_blobs = np.reshape(sol_precond[0: 3*self.Nblobs], (self.
      Nblobs, 3))

      # Save velocity
      #name = read.output_name + '.velocity.dat'
      np.savetxt('velocity_ellipsoid_bacteria_blobs.dat', velocities, delimiter='  ')

    
      # Save blob_forces
      #name = read.output_name + '.blob_forces.dat'
      np.savetxt('lambda_bacteria_blobs.dat', lambda_blobs, delimiter='  ')
      ...
      order_multipoles = self.order_multipoles
      
      
      Mn = []
      #if order_multipoles < 0:
      #  return U
      if order_multipoles >= 0:
        M0 = np.sum(lambda_blobs, axis=0)
        M0 = np.reshape(M0, (1,3))
        Mn.append(M0)
      if order_multipoles >= 1:
        M1 = np.einsum('bi,bj->ij', r_vectors_blobs-b.location, lambda_blobs)
        M1 = np.reshape(M1, (3,3))        
        Mn.append(M1)     
      if order_multipoles >= 2:
        M2 = np.einsum('bi,bj,bk->ijk', r_vectors_blobs-b.location, r_vectors_blobs-b.location, lambda_blobs)
        M2 = np.reshape(M2, (9,3))
        Mn.append(M2) 
      if order_multipoles >= 3:
        M3 = np.einsum('bi,bj,bk,bl->ijkl',r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, lambda_blobs)
        M3 = np.reshape(M3, (27,3))
        Mn.append(M3)
      if order_multipoles >= 4:
        M4 = np.einsum('bi,bj,bk,bl,bm->ijklm',r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, lambda_blobs)
        M4 = np.reshape(M4, (81,3))
        Mn.append(M4)
      if order_multipoles >= 5:
        M5 = np.einsum('bi,bj,bk,bl,bm,bn->ijklmn',r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, lambda_blobs)
        M5 = np.reshape(M5, (243, 3))
        Mn.append(M5)
      if order_multipoles >= 6:
        M6 = np.einsum('bi,bj,bk,bl,bm,bn,bo->ijklmno',r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, lambda_blobs)
        M6 = np.reshape(M6, (729, 3))
        Mn.append(M6)
      if order_multipoles >= 7:
        M7 = np.einsum('bi,bj,bk,bl,bm,bn,bo,bp->ijklmnop',r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, r_vectors_blobs-b.location, lambda_blobs)
        M7 = np.reshape(M7, (2187, 3))
        Mn.append(M7)  
          
      Mn = np.concatenate(Mn)
      
      #print(Mn)
  
      
      
      location_bacteria = b.location
      orientation_bacteria = b.orientation
      
      #print('location of bacteria', location_bacteria)
      #print('orientation of bacteria', orientation_bacteria)
      
      
      '''
      
      
     # orientation_bacteria = Quaternion(orientation_bacteria / np.linalg.norm(orientation_bacteria))
      b_bacteria= body.Body(b.location, orientation_bacteria, r_vectors_blobs, self.blob_radius)
      
     # Here we set the implementation to compute the blob mobility
      b_bacteria.mobility_blobs = mob.rotne_prager_tensor

      M_bacteria = b_bacteria.calc_mobility_blobs(self.eta, self.blob_radius)
      #print('M_bacteria shape = ', M_bacteria.shape)
      L_bacteria, lower = scipy.linalg.cho_factor(M_bacteria)
      L_bacteria = np.triu(L_bacteria)  
      M_inv_bacteria = scipy.linalg.solve_triangular(L_bacteria, scipy.linalg.solve_triangular(L_bacteria, np.eye(b_bacteria.Nblobs * 3), trans='T', check_finite=False), check_finite=False)
  
      #print('M_inv_bacteria shape = ', M_inv_bacteria.shape)
      K_bacteria = b_bacteria.calc_K_matrix()
      #print('K_bacteria shape = ', K_bacteria.shape)
  
      ###### #####  calculation of N = (K^t M^-1 K)^-1 ########
      N_bacteria = np.linalg.pinv(np.dot(K_bacteria.T, scipy.linalg.cho_solve((L_bacteria,lower), K_bacteria, check_finite=False)))
  

      #####  calculation of NK^tM^-1 ######## 
      tot_mob_matrix_bacteria = np.dot(N_bacteria, np.dot(K_bacteria.T, M_inv_bacteria))
      ...
      print('slip vel of bacteria', slip)  
      
      '''
        
      #U_bacteria = -np.dot(tot_mob_matrix_bacteria, slip.flatten())
      #print('U_bacteria shape = ', U_bacteria.shape)
      U_bacteria = self.U_bacteria
      U_bacteria[0,0] = mean((velocities[0],velocities[6]))
      U_bacteria[0,1] = mean((velocities[1],velocities[7]))
      U_bacteria[0,2] = mean((velocities[2],velocities[8]))
      U_bacteria[1,0] = mean((velocities[3],velocities[9]))
      U_bacteria[1,1] = mean((velocities[4],velocities[10]))
      U_bacteria[1,2] = mean((velocities[5],velocities[11]))
      #print(U_bacteria) 
      #U_bacteria = np.reshape(velocities, (4,3))  
      ...
      #print(step)
      X_bacteria = np.row_stack((U_bacteria, Mn / self.eta))
      
      #print(X_bacteria)
      #X_bacteria_averaged = np.zeros((X_bacteria.shape))
      #print('self before', self.X_bacteria_averaged)
      self.U_bacteria_averaged = self.U_bacteria_averaged + velocities
      self.X_bacteria_averaged = self.X_bacteria_averaged + X_bacteria
      #self.X_bacteria_averaged = (X_bacteria+self.X_bacteria_averaged)/(step+1)
      #print('self after', self.X_bacteria_averaged)
      #print('just X_bacteria_aeraged',self.X_bacteria_averaged)
      
     
      #if (step+1 % self.n_steps == self.n_steps):
      if ((step+1) % self.n_steps) == 0 and step >= 0: 
        print('n_steps = ', self.n_steps)
        self.X_bacteria_averaged = self.X_bacteria_averaged/self.n_steps
        self.U_bacteria_averaged = self.U_bacteria_averaged/self.n_steps
        #print('after dividing', self.X_bacteria_averaged)
        print(self.order_multipoles)
        name_X = 'X_sphere_bacteria_averaged_' + str(self.order_multipoles) + '.dat'
        np.savetxt(name_X, self.X_bacteria_averaged, delimiter='  ')
        np.savetxt('U_sphere_bacteria_averaged.dat', self.U_bacteria_averaged, delimiter='  ')    
      ...
      if self.plot_velocity_field.size > 1: 
        ...
        #pvf.plot_velocity_field(self.plot_velocity_field, r_vectors_blobs, lambda_blobs, self.blob_radius, self.eta, self.output_name, self.tracer_radius,
        #                  mobility_vector_prod_implementation = self.mobility_vector_prod_implementation, periodic_length=self.periodic_length)
        pvf.plot_velocity_field(r_vectors_blobs, lambda_blobs, self.blob_radius, self.eta, self.output_name, self.tracer_radius,
                          mobility_vector_prod_implementation = self.mobility_vector_prod_implementation, periodic_length=self.periodic_length)

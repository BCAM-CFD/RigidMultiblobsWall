...
class Lub_Solver(object):
  ...
  def Set_R_Mats(self, r_vecs_np=None):
    ...
    rows = [];
    cols = [];
    self.LC.ResistCOO(r_vecs,neighbors,self.a,self.eta,self.cutoff,self.cutoff_wall,self.periodic_length,False,data,rows,cols)
    if data:
      R_MB_coo_cut = sp.coo_matrix((data, (rows, cols)), shape=(6*num_particles, 6*num_particles)) 
      R_MB_cut = R_MB_coo_cut.tocsc()
    else:
      R_MB_cut = sp.diags(small*np.ones(6*num_particles),0,format='csc') #sp.csc_matrix(shape=(6*num_particles, 6*num_particles))
    
    data = [];
    rows = [];
    cols = [];
    self.LC.ResistCOO_wall(r_vecs,self.a,self.eta,self.cutoff_wall,self.periodic_length,False,data,rows,cols)
    if data:
      R_MB_coo_cut_wall = sp.coo_matrix((data, (rows, cols)), shape=(6*num_particles, 6*num_particles)) 
      R_MB_cut_wall = R_MB_coo_cut_wall.tocsc()
    else: 
      R_MB_cut_wall = sp.diags(small*np.ones(6*num_particles),0,format='csc') #sp.csc_matrix(shape=(6*num_particles, 6*num_particles))
    ...
    rows = [];
    cols = [];
    self.LC.ResistCOO(r_vecs,neighbors,self.a,self.eta,self.cutoff,self.cutoff_wall,self.periodic_length,True,data,rows,cols)
    if data:
      R_sup_coo_cut = sp.coo_matrix((data, (rows, cols)), shape=(6*num_particles, 6*num_particles)) 
      R_sup_cut = R_sup_coo_cut.tocsc()
    else:
      R_sup_cut = sp.diags(small*np.ones(6*num_particles),0,format='csc') #sp.csc_matrix(shape=(6*num_particles, 6*num_particles))
    
    data = [];
    rows = [];
    cols = [];
    self.LC.ResistCOO_wall(r_vecs,self.a,self.eta,self.cutoff_wall,self.periodic_length,True,data,rows,cols)
    if data:
      R_sup_coo_cut_wall = sp.coo_matrix((data, (rows, cols)), shape=(6*num_particles, 6*num_particles)) 
      R_sup_cut_wall = R_sup_coo_cut_wall.tocsc()
    else:
      R_sup_cut_wall = sp.diags(small*np.ones(6*num_particles),0,format='csc') #sp.csc_matrix(shape=(6*num_particles, 6*num_particles))
  ...
  def Wall_Mobility_Mult(self, X, r_vecs_np=None):
    ...
    U = self.mobility_trans_times_force_wall(r_vecs, F, self.eta, self.a, periodic_length=self.periodic_length) + self.mobility_trans_times_torque_wall(r_vecs, T, self.eta, self.a, periodic_length=self.periodic_length)
    W = self.mobility_rot_times_force_wall(r_vecs, F, self.eta, self.a, periodic_length=self.periodic_length) + self.mobility_rot_times_torque_wall(r_vecs, T, self.eta, self.a, periodic_length=self.periodic_length)
  ...
  def Lub_Mobility_Centered_RFD(self, W = None):
    ...
    r_vecs = self.put_r_vecs_in_periodic_box(r_vecs_np,self.periodic_length)
    if W is None:
      W = np.random.randn(6*r_vecs.shape[0]) #np.random.uniform(-1.0,1.0,6*r_vecs.shape[0]) #
  ...
  def Update_Bodies(self, FT):
    ...
    for k, b in enumerate(self.bodies):
        ...
        quaternion_dt = Quaternion.from_rotation((velocities[6*k+3:6*k+6]) * self.dt)
  ...
  def Update_Bodies_Trap(self, FT_calc, Omega=None, Out_Torque=False, Cut_Torque=None):
    ...
    for k, b in enumerate(self.bodies):
        ...
        quaternion_dt = Quaternion.from_rotation((vel_trap[6*k+3:6*k+6]) * self.dt)
